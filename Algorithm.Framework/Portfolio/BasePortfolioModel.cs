...
namespace QuantConnect.Algorithm.Framework.Portfolio
{
    ...
    /// </summary>
    public abstract class BasePortfolioModel : PortfolioConstructionModel
    {
        ...
        public override IEnumerable<IPortfolioTarget> CreateTargets(QCAlgorithmFramework algorithm, Insight[] insights)
        {
            ...
            foreach (var symbol in symbols)
            {
                ...
                {
                    var symbolTargets = CloseTargetsFromInsight(algorithm, symbol, activeInsights.Last());
                    if (symbolTargets == null || symbolTargets.Count == 0)
                    {
                        symbolTargets = OpenTargetsFromInsight(algorithm, symbol, activeInsights.Last());
                    }
                    if (symbolTargets != null)
                    {
                        targets.AddRange(symbolTargets);
                    }
                }
            }
